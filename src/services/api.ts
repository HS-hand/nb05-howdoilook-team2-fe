...
export const putComment = async (commentId: string, body: CommentFormInput) => {
...
export const postCurating = async (styleId: string, body: CuratingFormInput) => {
...
export const getCuratings = async (
  ...
  const { currentPage, totalPages, totalItemCount, data } = await response.json()
  //console.log(data);
...
export const deleteCurating = async (
  ...
  const data = await response.json()
  if (!response.ok) {
    throw new Error(data.message || '큐레이팅 삭제에 실패했습니다.')
  }
  return data
}
...
export const getStyle = async (styleId: string): Promise<StyleDetail> => {
  const response = await fetch(`${BASE_URL}/styles/${styleId}`)
  const style = await response.json()
  return style
}

export const putStyle = async (styleId: string, body: StyleFormInput) => {
  const response = await fetch(`${BASE_URL}/styles/${styleId}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
  ...
  const data = await response.json()
  if (!response.ok) {
    throw new Error(data.message || '스타일 게시글 수정에 실패했습니다.')
  }
  return data
}
...
export const deleteStyle = async (
  ...
  if (!response.ok) {
    throw new Error(message.message || '스타일 게시글 삭제에 실패했습니다.')
  }
  return { message }
}
